...
│   │   ├── banner_fetcher.py     # Wiki からのバナー画像取得（FR-BAN-006〜007, v2.0）
...
**位置づけ**: SELECT 画面のカーソル楽曲特定および RESULT 画面 OCR フォールバックを目的とした、楽曲特定の **2 次認識器**（§4.5.2）。PoC #04 v0.3 で方式採用確定。
...
#### 9.11.2. 配信パイプライン（FR-BAN-004）

```
[ scripts/build_banner_master.py ]  (開発者環境で生成)
   ├── Wiki MediaWiki API 取得（remywiki + fandom）
   ├── CS_pnm_Lively ページ起点で楽曲ページ抽出
   ├── 各楽曲ページの Song Information から原タイトル取得
   ├── master.json と突合 (genre / alt_titles[] と併用)
   ├── 画像 DL → 390×94 リサイズ正規化 → pHash/dHash 算出
   └── data/banner_features.json 出力

[ GitHub Releases ]
   ├── master.json
   └── banner_features.json  (独立配布、ETag 対応)

[ アプリ起動時 ]
   BannerFeatureFetcher.fetch() → BannerMatchService.load() → メモリ常駐
```
...
#### 9.11.5. 著作権・運用配慮（FR-BAN-005〜008, NFR-LEGAL-001/004/005）

- **画像本体は配布物に一切含めない**: リポジトリ・PyInstaller 出力・GitHub Releases いずれにもバナー画像 PNG を含めない。`poc_out/` 同様 `.gitignore` 必須。
- **ユーザ PC 同意取得**: 設定画面に「Wiki からバナー画像を取得」ボタン。初回押下時に同意ダイアログ（取得元、用途、私的複製の範囲、ユーザ責任）。
- **Wiki アクセスマナー**: User-Agent に連絡先 URL、レート ≥ 1.0 秒、`robots.txt` 尊重、再 DL なし（キャッシュ優先）。
- **属性表示**: About 欄に「バナー画像参照: remywiki.com / popnmusic.fandom.com」「本アプリは KONAMI 公式とは無関係です」。
- **オプトアウト**: 設定で「Wiki 自動収集無効化」可。無効時は同梱 `banner_features.json` のみで動作。
