...
[ scripts/build_banner_master.py ]  (★ 開発者専用ツール。エンドユーザは実行しない)
   ├── 開発者が用意したバナー画像を入力（開発者環境のみ）
   ├── master.json と突合 (genre / alt_titles[] と併用)
   ├── 390×94 リサイズ正規化 → pHash/dHash 算出
...
- **免責表示**: About 欄に「本アプリは KONAMI 公式とは無関係です」の免責を表示。
- **特徴量マスタ生成は開発者環境のみ**: バナー画像の取り扱いは **開発者ツール `build_banner_master.py` 側の責務**とし、生成済みの数値ハッシュのみを配布する。
- **同意ダイアログ・オプトアウト設定は廃止**: アプリはユーザ PC で画像の取得・保存を一切行わないため、同意取得・有効/無効切替の UI は存在しない。
...
| 0.6 | 2026-05-29 | 要件 v0.8 反映: §1.3 から `banner_fetcher.py` 削除、§9.11.2 パイプライン図に「開発者専用」「画像本体はアプリ・配布物の両方に含めない」を明示、§9.11.5 から同意ダイアログ・自動収集オプトアウトの設計を撤去（アプリは画像の取得・保存を一切行わない方針） | Claude Code |
