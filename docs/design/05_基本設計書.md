...
│   ├── application/              # サービス（RecordingService、BannerMatchService 等）
│   ├── domain/
...
│   │   │   ├── base.py
│   │   │   ├── select_screen.py  # SELECT 画面（FR-BAN-002, v2.0 強化）
│   │   │   ├── ready_screen.py
│   │   │   ├── option_screen.py
│   │   │   ├── play_screen.py
│   │   │   ├── result_screen.py  # RESULT 画面（FR-BAN-001, v2.0 強化）
│   │   │   └── load_screen.py
│   │   ├── banner_features.py    # pHash/dHash 計算ユーティリティ（v2.0）
│   │   ├── ocr/
│   │   │   ├── base.py
│   │   │   ├── paddle.py
│   │   │   └── tesseract.py
...
│   └── build_banner_master.py    # バナー特徴量マスタ生成（FR-BAN-011, v2.0）
├── data/
│   ├── master.json               # 楽曲マスタ（既存）
│   └── banner_features.json      # バナー特徴量マスタ（FR-BAN-003, v2.0）
├── tests/                        # pytest
└── docs/                         # 本ドキュメント群
```
...
| 楽曲名・難易度 | バナー上部の「difficulty + song banner」 | 難易度ラベル：色＋テンプレ、楽曲名：プレイ画面で取得済みのキャッシュを優先採用 → **2次認識器: バナー特徴量マッチ**（§9.11、v2.0） |
| BEST SCORE 差分 | 「BEST SCORE - / +」 | OCR（参考情報） |

> 楽曲名はリザルト画面のバナー画像（装飾あり・OCRが難）よりも、**直前のプレイ画面で取得済みの楽曲名** を優先する設計とする（R-001 への対応）。v2.0 ではキャッシュ未取得・低信頼時のフォールバックとして **バナー特徴量マッチ**（§9.11）を組み合わせる。

### 4.5. 楽曲特定アルゴリズム

#### 4.5.1. 1 次認識器: OCR + ファジーマッチ（v1.x 既存）
...
#### 4.5.2. 2 次認識器: バナー特徴量マッチ（v2.0、§9.11）

OCR ベースの 1 次認識器で「未特定」あるいは信頼度しきい値未満の場合に、**SELECT/RESULT 画面の楽曲バナー領域から計算した pHash + dHash** を `data/banner_features.json` と照合する。

```
入力: フレーム frame_bgr, 画面種別 screen ∈ {SELECT, RESULT}
1. screen に応じた ROI（SELECT: READY_ROI["song_banner"] / RESULT: RESULT_ROI["banner"]）を切り出し
2. 390×94 にリサイズ正規化 → グレースケール化
3. pHash(64bit) と dHash(64bit) を計算
4. banner_features.json 上の全エントリと、pHash+dHash 合算ハミング距離で Top-K(K=5)
5. 距離 ≤ accept_threshold (= 20 / 128bit, 詳細設計で再キャリブレーション) → 確定
6. accept_threshold < 距離 ≤ candidate_threshold (= 40) → 1 次認識器の上位候補と突合し、共通候補があれば確定
7. それ以外は「未特定」のまま 1 次認識器に委譲
```

しきい値は PoC #04 のノイズフロア実測（正解未含有時の Top-1 距離 22〜30）を出発点に、本実装フェーズで lively 楽曲の正解付き測定で再決定する。
...
### 9.11. バナー画像認識（v2.0、FR-BAN-001〜011）

**位置づけ**: SELECT 画面のカーソル楽曲特定および RESULT 画面 OCR フォールバックを目的とした、楽曲特定の **2 次認識器**（§4.5.2）。PoC #04 v0.3 で方式採用確定、要件定義書 v0.8 でバナー画像本体をアプリのランタイム動作からも完全に排除。

#### 9.11.1. データ構造（FR-BAN-003）

`data/banner_features.json`:

```json
{
  "version": "2026-MM-DDTHH:MM:SSZ",
  "source": "https://github.com/OWNER/lively-/releases/...",
  "target_size": [390, 94],
  "songs": [
    { "song_id": "popn-xxxxxxxx", "phash": 1234567890123456789,
      "dhash": 9876543210987654321, "src": "remywiki:File:..." }
  ]
}
```

- SQLite スキーマは **変更しない**（PoC §11.1 PO 判断 案 C）。
- 起動時に `BannerMatchService` がメモリへロード（1347 件想定で約 22KB）。

#### 9.11.2. 配信パイプライン（FR-BAN-004 / FR-BAN-011）

```
[ scripts/build_banner_master.py ]  (★ 開発者専用ツール。エンドユーザは実行しない)
   ├── Wiki MediaWiki API 取得（remywiki + fandom、開発者環境のみ）
   ├── CS_pnm_Lively ページ起点で楽曲ページ抽出
   ├── 各楽曲ページの Song Information から原タイトル取得
   ├── master.json と突合 (genre / alt_titles[] と併用)
   ├── 画像 DL（開発者ローカル）→ 390×94 リサイズ正規化 → pHash/dHash 算出
   └── data/banner_features.json 出力（画像本体はコミットせず .gitignore）

[ GitHub Releases ]
   ├── master.json
   └── banner_features.json  (★ 数値ハッシュのみ。画像本体は含まない)

[ アプリ起動時 ]
   BannerFeaturesFetcher.fetch() → BannerMatchService.load() → メモリ常駐
   (★ アプリは画像本体を一切扱わない。ユーザ PC への画像保存も発生しない)
```

#### 9.11.3. マッチング（FR-BAN-001, FR-BAN-002）

`BannerMatchService.identify(frame_bgr, screen)`:

1. screen ∈ {SELECT, RESULT} に応じた ROI 切り出し（§10 詳細設計）
2. 390×94 リサイズ → グレースケール → pHash/dHash 計算
3. メモリ上の全エントリと「pHash 距離 + dHash 距離」（128bit 中のハミング距離合計）で Top-K(K=5) 取得
4. しきい値判定（§4.5.2）で `IdentifyResult(song_id, score, confidence)` を返す

#### 9.11.4. 性能予算

| 指標 | PoC 実測 | 本実装予算 (NFR-PERF-BAN-001) |
|------|----------|------------------------------|
| pHash 計算 / 枚 | 0.089 ms | — |
| Top-5 over 1347 件 | 0.357 ms | ≤ 30 ms |
| 合計 / 枚 | < 1 ms | ≤ 30 ms |

→ 認識ワーカ 2 fps（NFR-PERF-002）に対し過剰に余裕。SELECT 画面でカーソル移動毎に呼び出しても問題なし。

#### 9.11.5. 著作権・運用配慮（FR-BAN-005 / FR-BAN-008, NFR-LEGAL-001/004/005）

- **画像本体は配布物にもアプリにも一切含めない**:
  - リポジトリ・PyInstaller 出力・GitHub Releases にバナー画像 PNG を含めない。
  - **アプリのランタイム動作でユーザ PC に画像本体を保存することもしない**（v0.8 で強化）。
  - 取得・保存・キャッシュの実装はアプリ層に存在しない。
- **属性表示**: About 欄に「バナー画像参照: remywiki.com / popnmusic.fandom.com」「本アプリは KONAMI 公式とは無関係です」（特徴量マスタの生成源として参照した Wiki の出典明示）。
- **Wiki アクセスマナー**は **開発者ツール `build_banner_master.py` 側の責務**: UA に連絡先 URL、レート ≥ 1.0 秒、`robots.txt` 尊重、再 DL なし。開発者が手元で実行する際に遵守。
- **同意ダイアログ・オプトアウト設定は廃止**: アプリはユーザ PC で Wiki アクセスを一切行わないため、同意取得・有効/無効切替の UI は存在しない。

#### 9.11.6. 失敗時の挙動

- `banner_features.json` 取得失敗 → 既存 OCR 1 次認識器のみで継続。アプリ起動はブロックしない。
- バナー領域 ROI が範囲外（解像度不一致等）→ WARN ログ＋本機能スキップ、本体動作は継続。
- マッチ結果が `accept_threshold` 未満 → 「未特定」のまま 1 次認識器に委譲（フォールバック先存在）。
...
| 14.8 | バナー認識（§9.11）の詳細実装 | 詳細設計書 `11_詳細設計_バナー認識.md` で別途規定（PoC #04 v0.3 確定方針に基づく） |
...
| 0.5 | 2026-05-29 | v2.0 機能追加（要件 v0.7 反映）: §1.3 にバナー認識系モジュール（banner_fetcher、banner_features、build_banner_master、data/banner_features.json）追加、§4.4/4.5 に 2 次認識器（バナー特徴量マッチ）を追記、§9.11 バナー画像認識を新設、§14.8 詳細設計持ち越し追加。PoC #04 v0.3 PO 確定方針を反映 | Claude Code |
| 0.6 | 2026-05-29 | 要件 v0.8 反映: §1.3 から `banner_fetcher.py` 削除、§9.11.2 パイプライン図に「開発者専用」「画像本体はアプリ・配布物の両方に含めない」を明示、§9.11.5 から同意ダイアログ・Wiki 自動収集オプトアウトの設計を撤去（アプリは Wiki アクセスを一切行わない方針） | Claude Code |
