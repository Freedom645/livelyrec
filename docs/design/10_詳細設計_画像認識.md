...
flowchart TD
    ...
    F --> I
    I --> J[結果出力]
    F -->|スコア安定| K[ResultWriter.save<br>FR-REC-046]
    F -->|スコア安定 + 開発者設定ON| L[BannerWriter.save<br>FR-DEV-002]
```
...
RESULT_ROI = {
    ...
    "banner":      (489, 233, 879, 327),  # 開発者向けバナー画像クロップ領域（FR-DEV-002、PO 指定 x=489,y=233,w=390,h=94）
}
```

> `banner` ROI は PO 指定値 `x=489, y=233, width=390, height=94`（1366×768 正規化座標系）を採用。`roi_defs.py` の `(x1, y1, x2, y2)` 形式に変換した結果が上記。
...
### 6.4. 検出失敗の確定（FR-REC-039）

「特定不能」と「まだ判定が走っていない」を区別するため、楽曲名 OCR の呼び出し回数を追跡する。

- I-027 対応で楽曲名 OCR はプレイ開始直後の数フレームに限定されている（`pipeline.analyze(song_already_identified=False)` のときのみ実行）。
- `identify_chart()` が連続 **5 回**（または楽曲名 OCR が走った全フレーム）で `None` を返した場合、当該プレイは **検出失敗確定** とする。
- 確定後は `AnalysisResult.song_identification_failed = True` を立てて上流に通知。`RecordingService` は `chart_id = NULL` の `play_session` レコードを作成し、`now_playing.changed` を「display_title="検出失敗", identified=false」で配信する。
- 検出失敗確定後はそのプレイ中、楽曲名 OCR を再試行しない（OCR 呼び出し回数の抑制、I-027 方針との整合）。

```python
class SongIdentificationTracker:
    def __init__(self, fail_after: int = 5) -> None: ...
    def record_attempt(self, result: Chart | None) -> None: ...
    def is_confirmed_failed(self) -> bool: ...
    def reset(self) -> None: ...   # 新セッション開始時
```
...
| Result | banner | 489 | 233 | 879 | 327 | クロップのみ | 開発者向けバナー画像出力（FR-DEV-002、PO 指定: x=489,y=233,w=390,h=94） |
...
| 0.2 | 2026-05-27 | v1.x 機能追加: RESULT_ROI に banner を追加（FR-DEV-002 PO 指定値 x=489,y=233,w=390,h=94）、§1 パイプライン図に ResultWriter / BannerWriter 呼び出しを追記、§6.4 検出失敗の確定ロジック（SongIdentificationTracker）を追加 | Claude Code |
