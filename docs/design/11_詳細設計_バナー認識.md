...
| 版数 | 0.2 |
| 作成日 | 2026-05-29 |
| 関連資料 | 要件定義書 v0.8 §3.8、基本設計書 v0.6 §4.5.2／§9.11、PoC #04 v0.3（`docs/design/poc/04_banner_recognition.md`）、詳細設計_画像認識（`10_詳細設計_画像認識.md`） |
...
スコープ:
- 特徴量データのスキーマ・配布方式
- 収集パイプライン（`scripts/build_banner_master.py`、**開発者専用ツール**）
- ランタイム側マッチングサービス（`BannerMatchService`）
- SELECT / RESULT 画面認識への組込み
- 失敗時の挙動・しきい値設計
- UI 連携（About 出典表示、設定タブ）

スコープ外（別文書）:
- 楽曲マスタの拡張仕様（`master.json` の `genre` / `alt_titles[]`）は `A-2 残課題` として `scripts/build_master.py` 側の改修。本書では呼び出し前提のみ規定。

**v0.2 で撤去された機能（要件 v0.8 反映、画像本体をアプリで一切扱わない方針）:**
- アプリ内からの Wiki 画像取得（`livelyrec_data/banners_ref/` ローカルキャッシュ含む）
- 同意ダイアログ（`BannerConsentDialog`）
- 設定の `banner.auto_fetch_enabled` / `banner.cache_dir`
...
## 4. 収集パイプライン（`scripts/build_banner_master.py`、開発者専用ツール）

**位置づけ**: 本スクリプトは **開発者環境でのみ実行する内部ツール** であり、エンドユーザは実行しない。アプリ配布物には含めない場合もあるが、リポジトリには配置する（再現性確保のため）。生成された `data/banner_features.json`（数値ハッシュのみ）を GitHub Releases に同梱配布する。
...
### 4.4. レート制御・キャッシュ（開発者環境）

- リクエスト間隔: 既定 1.0 秒（`--rate-sec` で調整可、最小 0.5）
- User-Agent: `LivelyRec/<version> (banner-master builder; +<contact url>)`
- ローカルキャッシュ: 開発者の作業環境上の任意ディレクトリ（`--cache-dir` で指定、既定は `poc_out/banners_ref/`）に画像本体を一時保存。再実行時は既存ファイル優先（再 DL なし）。**配布物・リポジトリには含めず `.gitignore` で除外する**
...
infrastructure/
  ...
  github_client.py              # 既存。BannerFeaturesFetcher を含む
                                # data/banner_features.json の取得（GitHub Releases）
```

**廃止**: `banner_image_fetcher.py`（v0.1 で計画していたユーザ操作による Wiki 画像取得モジュール）は v0.2 で撤去。アプリは画像本体を一切扱わない。
...
v0.1 で計画していた `BannerConsentDialog` は **v0.2 で撤去**。
アプリはユーザ PC で Wiki 画像取得を行わないため、同意取得 UI そのものが不要となった。
...
設定ダイアログの「楽曲認識」タブには以下のみ残す:

| 項目 | 既定 | 説明 |
|------|------|------|
| バナー特徴量マッチを使用する | ON | OFF 時は 1 次認識器（OCR）のみ |
| 特徴量配信URL | （空） | `banner_features.json` の配信エンドポイント。空時は同梱 seed を使用 |

設定キー: `banner.match_enabled`, `banner.endpoint_url`。

**廃止された項目（v0.2）**:
- `banner.auto_fetch_enabled` / `banner.cache_dir`（Wiki 画像取得機能廃止に伴い不要）
...
**廃止された項目（v0.2）**: Wiki 画像取得・ローカルキャッシュ書込関連のエラーパスはアプリ層に存在しない。開発者ツール `build_banner_master.py` 側で同等のハンドリングを行う（同スクリプトの実装を参照）。
...
- v1.x → v2.0 でアプリ起動時:
  ...
  - 設定 `banner.endpoint_url` 未存在時は空文字（既定、同梱 seed のみで動作）。
- SQLite スキーマ変更なし、`schema_version` 据え置き（v2 のまま）。
- **廃止された設定**: `banner.auto_fetch_enabled` / `banner.cache_dir` は読み込みも書込みも行わない（旧設定ファイルに存在しても無視）。
...
| 0.2 | 2026-05-29 | 要件 v0.8 反映: アプリのランタイム動作からバナー画像本体を完全に排除する方針へ変更。§4 を「開発者専用ツール」と明示、§5.1 から `banner_image_fetcher.py` 削除、§7.1 同意ダイアログ撤去、§7.3 設定タブから `auto_fetch_enabled` / `cache_dir` 削除、§8 エラーハンドリングから Wiki 画像取得関連削除、§11 マイグレーションに廃止設定の扱い追記 | Claude Code |
