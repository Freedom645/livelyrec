...
`schema_version` を起動時に確認し、必要なら旧スキーマ→新スキーマの変換を実施（v1 時点では不要）。
...
def main() -> int:
    ...
    rec = RecordingService(obs, analysis, repos.session, repos.daily, ws)
...
class AppPaths:
    ...
    def detect(cls) -> "AppPaths":
        ...
        for sub in ("db", "logs", "export", "crash"):
...
開発専用は `requirements-dev.txt`（pytest, pytest-qt, ruff, mypy, pyinstaller）に分離。
